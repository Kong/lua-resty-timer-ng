...
function _M.new(wheel_setting, resolution, on_expire)
    assert(type(wheel_setting) == "table", "wheel_setting must be a table")
    assert(type(resolution) == "number", "resolution must be a number")
    ...
    for level, slots in ipairs(wheel_setting.slots_for_each_level) do
        ...
        cur_wheel = wheel.new(wheel_id, slots, on_expire)
