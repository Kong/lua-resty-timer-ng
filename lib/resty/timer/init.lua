...
local util_module = require("resty.timer.util")
...
local function update_closet(self)
    ...
    for i = 1, 9 do
        ...
        if not util_module:is_empty_table(jobs) then
            break
        end
    end
...
local function update_all_wheels(self)
    ...
    while util_module:float_compare(self.real_time, self.expected_time) == 1 do
...
local function mover_timer_callback(premature, self)
    ...
    while not exiting() and not self.destory do
        ...
        local is_no_pending_jobs = util_module:is_empty_table(wheels.pending_jobs)
        local is_no_ready_jobs = util_module:is_empty_table(wheels.ready_jobs)
...
local function worker_timer_callback(premature, self, thread_index)
    ...
    while not exiting() and not self.destory do
        ...
        while not util_module:is_empty_table(wheels.pending_jobs) do
            thread.counter.runs = thread.counter.runs + 1

            local job = util_module:get_a_item_from_table(wheels.pending_jobs)
        ...
        if not util_module:is_empty_table(wheels.ready_jobs) then
